	...
	*/
	...
	if(pMainFrame->m_bShowLayerMap[layer_Agent_position])
	{
				...
				for (int i = 0; i<(*itr2).second.AgentLocationRecordVector.size()-1; i++)
				{
					...
					from_point = NPtoSP(from_pt);
					 to_point = NPtoSP(to_pt);
						...
						//str_text.Format("%s", (*itr2).second.AgentLocationRecordVector[i].agent_id.c_str());
						//				
						//CPoint screen_point = NPtoSP(text_pt);
						//pDC->TextOut(screen_point.x, screen_point.y, str_text);
		...
		pDC->SelectObject(&g_GPSTrajectoryColor);
			...
			if(pDoc->m_AgentLocationMap.find(timestamp_in_second) != pDoc->m_AgentLocationMap.end())
			{
				//with Agent location data at this time stamp
				...
				for(itr = pDoc->m_AgentLocationMap[timestamp_in_second].AgentLocationRecordVector .begin();
					itr != pDoc->m_AgentLocationMap[timestamp_in_second].AgentLocationRecordVector .end(); itr++)
				{		//scan all Agent records at this timestamp
	...
	if(pMainFrame->m_bShowLayerMap[layer_ODMatrix])
	{
		...
		for(i=0; i < pDoc->m_ZoneNoSize ; i++)
			...
			{
				if( pDoc->m_ODMOEMatrix[p][i][j].TotalAgentSize > MaxODDemand)
				{
					MaxODDemand = pDoc->m_ODMOEMatrix[p][i][j].TotalAgentSize;

				}

				if( bODDataReady == false && pDoc->m_ODMOEMatrix[p][i][j].TotalAgentSize >=10 && i!=j  )
				{
					pDoc->m_OD_data_vector.push_back (pDoc->m_ODMOEMatrix[p][i][j].TotalAgentSize);
				}
...
void CTLiteView::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	*/
	...
	if(pMainFrame-> m_iSelectedLayer == layer_Agent_position)
	{
		...
		if(st!=NULL)
		{
			...
			fprintf(st, "%d,%ld\n", pDoc->m_NodeIDMap[node_id]->m_NodeID, Time);
			fclose(st);
		}
	}
...
void CTLiteView::OnSearchFindlink()
{
	...
	if(dlg.DoModal () == IDOK)
	{
		...
		{
			int SelectedAgentID = dlg.m_AgentNumber; // internal Agent index starts from zero
...
void CTLiteView::DrawNodeMovements(CDC* pDC, DTANode* pNode, CRect PlotRect)
{
	...
	double size_ratio= pDoc->m_MovementTextBoxSizeInDistance/250;

	double node_set_back = size_ratio*150 * pDoc->m_UnitDistance;

	double link_length = size_ratio*250 * pDoc->m_UnitDistance;
	double movement_length = size_ratio*100 * pDoc->m_UnitDistance;
	double lane_width = size_ratio*10 * pDoc->m_UnitDistance;
