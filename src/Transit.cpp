...
//								link_length);
	...
	FILE* st = NULL;
	fopen_s(&st, m_PT_network.m_ProjectDirectory + "agent.csv", "w");
	if (st == NULL)
	{
		AfxMessageBox("Error: File agent.csv cannot be opened.\nIt might be currently used and locked by EXCEL.");
		return;
	}
	...
	if (st != NULL)
	{
		...
		AfxMessageBox("File agent.csv has been created.", MB_ICONINFORMATION);
